...
angular.module('bookingForm')
    .controller('BookingFormController', ['$http', '$state', '$stateParams', function ($http, $state, $stateParams) {
        var self = this;
            ...
            if (petId) { // edit
                $http.get("api/customer/owners/" + ownerId + "/pets/" + petId).then(function (resp) {
                   ...
                   // self.pet.birthDate = new Date(self.pet.birthDate);
                    self.pet.movieDate = new Date(self.pet.movieDate);
